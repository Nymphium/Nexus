...
fn test_array_module_fold_left_sum() {
    let src = &common::fixtures::read_test_fixture("test_array_module_fold_left_sum.nx");
    assert_eq!(run(src).unwrap(), Value::Int(10));
}

#[test]
fn test_array_module_find_index_any_all() {
    let src = &common::fixtures::read_test_fixture("test_array_module_find_index_any_all.nx");
    assert_eq!(run(src).unwrap(), Value::Int(2));
}

#[test]
fn test_array_module_map_in_place_and_for_each() {
    let src = &common::fixtures::read_test_fixture("test_array_module_map_in_place_and_for_each.nx");
    assert_eq!(run(src).unwrap(), Value::Int(6));
}

#[test]
fn test_array_module_filter_returns_list() {
    let src = &common::fixtures::read_test_fixture("test_array_module_filter_returns_list.nx");
    assert_eq!(run(src).unwrap(), Value::Int(32));
}

#[test]
fn test_array_module_partition_returns_two_lists() {
    let src = &common::fixtures::read_test_fixture("test_array_module_partition_returns_two_lists.nx");
    assert_eq!(run(src).unwrap(), Value::Int(22));
}

#[test]
fn test_array_module_zip_with_and_zip() {
    let src = &common::fixtures::read_test_fixture("test_array_module_zip_with_and_zip.nx");
